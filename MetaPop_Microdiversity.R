#Assembly name needs to be taken from the file; correct this down below when it's read in as assembled_contigs
#Same for genes
#Also the original codon calc/other stages where the microdiv data is looped through by row are terminally slow - should be parallel, think about ways to speed it up, too.
...
if(length(directory_name) > 1 | identical(check_for_default, directory_name)){
  print("Metapop requires the name of the directory where the folders gene_level_microdivput from preprocessing exist. Metapop will exit.")
  quitNow <- 1
}else{
...

{
   ...
   fwrite(microdiv_data, "metapop_microdiversity/raw_microdiversity_data_snp_loci_only.tsv", sep = "\t")
   
   fwrite(gene_level_microdiv, "metapop_microdiversity/gene_microdiversity.tsv", sep = "\t")
   fwrite(contig_level_microdiv, "metapop_microdiversity/contig_microdiversity.tsv", sep = "\t")
   
   fwrite(codon_pos_count, "metapop_microdiversity/codon_position_summary.tsv", sep = "\t")
