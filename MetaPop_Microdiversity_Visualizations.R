...
#codon_pos_count[,3:5] <- codon_pos_count[,3:5]/rowSums(codon_pos_count[,3:5])

#codon_pos_count <- melt.data.table(codon_pos_count, id.vars = c("source", "contig"))
...
pdf("metapop_visualizations/fst_genome_heatmap_plots.pdf", width = 17, height = 11)
...
gene_microdiv <- fread("metapop_microdiversity/gene_microdiversity.tsv", sep = "\t")
...
pdf(paste0("metapop_visualizations/contigs_bp_cat_",as.character(format(10^z, scientific = F)),"_microdiversity_viz.pdf"), width = 13 + (2^(z+1)), height = 11)
...
groups <- (1:length(unique_contigs)) %/% threads
unique_groups <- unique(groups)
